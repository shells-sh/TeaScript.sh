@spec.reflection.objects.setField() {
  local dogObjectId
  ...
  expect { reflection objects getField "$dogObjectId" name } toEqual "Rover"
  expect { reflection objects getField "$dogObjectId" age } toEqual 4
  expect { reflection objects show "$catObjectId" } toContain "Mittens"

  reflection objects setField "$catObjectId" name "Paws"
  expect { reflection objects getField "$catObjectId" name } toEqual "Paws"
  expect { reflection objects show "$catObjectId" } not toContain "Mittens"
}